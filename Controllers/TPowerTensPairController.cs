...
namespace TwoDPro3.Controllers
{
    ...
    public class TPowerTensPairController : Controller
    {
        private readonly CalendarContext _context;

        public TPowerTensPairController(CalendarContext context)
        {
            _context = context;
        }
        ...
        [HttpGet("alldaytpowertenspair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchAllDays(string tpowertenspair)
        {
            if (tpowertenspair != "tpowertenspair")
                return BadRequest("Parameter must be 'tpowertenspair'.");

            var foundRows = await _context.Table1
                .Where(c => TPowerNumbers.Contains(c.Am) && TensNumbers.Contains(c.Pm))
                .OrderBy(c => c.Id)
                .ToListAsync();
        ...
        [HttpGet("weeksettpowertenspair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchWeekSets(string tpowertenspair, string day)
        {
            if (tpowertenspair != "tpowertenspair")
                return BadRequest("Parameter must be 'tpowertenspair'.");
            ...
            var foundRows = await _context.Table1
                .Where(c => c.Days == day && TPowerNumbers.Contains(c.Am) && TensNumbers.Contains(c.Pm))
                .OrderBy(c => c.Id)
                .ToListAsync();
