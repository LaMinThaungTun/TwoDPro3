...
namespace TwoDPro3.Controllers
{
    ...
    public class DgOneOnePowerPairController : Controller
    {
        private readonly CalendarContext _context;

        public DgOneOnePowerPairController(CalendarContext context)
        {
            _context = context;
        }
        ...
        // ================= ALL DAYS =================
        [HttpGet("alldaydgoneonepowerpair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchAllDays(string dgoneonepowerpair)
        {
            if (dgoneonepowerpair != "dgoneonepowerpair")
                return BadRequest("Parameter must be 'dgoneonepowerpair'.");
            ...
            var foundRows = await _context.Table1
                .Where(c =>
                    ...
                    c.PmDgOne.Length == 1 &&
                    (
                        (c.AmDgOne.Substring(0, 1) == "0" && c.PmDgOne.Substring(0, 1) == "5") ||
                        (c.AmDgOne.Substring(0, 1) == "5" && c.PmDgOne.Substring(0, 1) == "0") ||

                        (c.AmDgOne.Substring(0, 1) == "1" && c.PmDgOne.Substring(0, 1) == "6") ||
                        (c.AmDgOne.Substring(0, 1) == "6" && c.PmDgOne.Substring(0, 1) == "1") ||

                        (c.AmDgOne.Substring(0, 1) == "2" && c.PmDgOne.Substring(0, 1) == "7") ||
                        (c.AmDgOne.Substring(0, 1) == "7" && c.PmDgOne.Substring(0, 1) == "2") ||

                        (c.AmDgOne.Substring(0, 1) == "3" && c.PmDgOne.Substring(0, 1) == "8") ||
                        (c.AmDgOne.Substring(0, 1) == "8" && c.PmDgOne.Substring(0, 1) == "3") ||

                        (c.AmDgOne.Substring(0, 1) == "4" && c.PmDgOne.Substring(0, 1) == "9") ||
                        (c.AmDgOne.Substring(0, 1) == "9" && c.PmDgOne.Substring(0, 1) == "4")
                    )
            ...
            if (!foundRows.Any())
                return NotFound("No matching DG One-One power pairs found.");
        ...
        // ================= WEEK SETS =================
        [HttpGet("weeksetsdgoneonepowerpair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchWeekSets(string dgoneonepowerpair, string day)
        {
            if (dgoneonepowerpair != "dgoneonepowerpair")
                return BadRequest("Parameter must be 'dgoneonepowerpair'.");
        ...
        private async Task<List<List<Calendar>>> GetFourWeekSetsAsync(List<Calendar> foundRows)
        {
            ...
            foreach (var row in foundRows)
            {
                ...
                foreach (var (y, w) in normalizedWeeks)
                {
                    ...
                    {
                        block.AddRange(
                            weekRows.OrderBy(c => DayOrder.ContainsKey(c.Days) ? DayOrder[c.Days] : 999)
                                    .ThenBy(c => c.Id)
                        );
                    }
                }
