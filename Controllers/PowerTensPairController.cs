...
namespace TwoDPro3.Controllers
{
    ...
    public class PowerTensPairController : Controller
    {
        private readonly CalendarContext _context;

        public PowerTensPairController(CalendarContext context)
        {
            _context = context;
        }
        ...
        [HttpGet("alldaypowertenspair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchAllDays(string powertenspair)
        {
            if (powertenspair != "powertenspair")
                return BadRequest("Parameter must be 'powertenspair'.");

            var foundRows = await _context.Table1
                .Where(c => PowerNumbers.Contains(c.Am) && TensNumbers.Contains(c.Pm))
                .OrderBy(c => c.Id)
                .ToListAsync();
        ...
        [HttpGet("weeksetpowertenspair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchWeekSets(string powertenspair, string day)
        {
            if (powertenspair != "powertenspair")
                return BadRequest("Parameter must be 'powertenspair'.");
            ...
            var foundRows = await _context.Table1
                .Where(c => c.Days == day && PowerNumbers.Contains(c.Am) && TensNumbers.Contains(c.Pm))
                .OrderBy(c => c.Id)
                .ToListAsync();
