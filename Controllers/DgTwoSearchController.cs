...
namespace TwoDPro3.Controllers
{
    ...
    public class DgTwoSearchController : Controller
    {
        private readonly CalendarContext _context;
        public DgTwoSearchController(CalendarContext context)
        {
            _context = context;
        }
        ...
        // Example: GET api/DgOneSearch/alldaysdgtwo?number=1
        [HttpGet("alldaysdgtwo")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchAllDays(string number)
        {
            var foundRows = await _context.Table1
                .Where(c => (c.AmDgTwo == number || c.PmDgTwo == number) && (c.Years == 2024 || c.Years == 2025 || c.Years == 2026))
                .OrderBy(c => c.Id)
                .ToListAsync();
            if (!foundRows.Any())
        ...
        public async Task<ActionResult<List<List<Calendar>>>> SearchWeekSets(string number, string day, bool am = false, bool pm = false)
        {
            ...
            if (am && pm)
            {
                query = query.Where(c => c.AmDgTwo == number || c.PmDgTwo == number);
            }
            else if (am)
            {
                query = query.Where(c => c.AmDgTwo == number);
            }
            else if (pm)
            {
                query = query.Where(c => c.PmDgTwo == number);
            }
            else
            {
