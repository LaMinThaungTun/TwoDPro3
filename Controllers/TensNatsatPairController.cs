...
namespace TwoDPro3.Controllers
{
    ...
    public class TensNatsatPairController : Controller
    {
        private readonly CalendarContext _context;

        public TensNatsatPairController(CalendarContext context)
        {
            _context = context;
        }
        ...
        [HttpGet("alldaytensnatsatpair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchAllDays(string tensnatsatpair)
        {
            if (tensnatsatpair != "tensnatsatpair")
                return BadRequest("Parameter must be 'tensnatsatpair'.");

            var foundRows = await _context.Table1
                .Where(c => TensNumbers.Contains(c.Am) && NatsatNumbers.Contains(c.Pm))
                .OrderBy(c => c.Id)
                .ToListAsync();
        ...
        [HttpGet("weeksettensnatsatpair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchWeekSets(string tensnatsatpair, string day)
        {
            if (tensnatsatpair != "tensnatsatpair")
                return BadRequest("Parameter must be 'tensnatsatpair'.");
            ...
            var foundRows = await _context.Table1
                .Where(c => c.Days == day && TensNumbers.Contains(c.Am) && NatsatNumbers.Contains(c.Pm))
                .OrderBy(c => c.Id)
                .ToListAsync();
