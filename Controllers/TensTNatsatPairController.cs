...
namespace TwoDPro3.Controllers
{
    ...
    public class TensTNatsatPairController : Controller
    {
        private readonly CalendarContext _context;

        public TensTNatsatPairController(CalendarContext context)
        {
            _context = context;
        }
        ...
        [HttpGet("alldaytenstnatsatpair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchAllDays(string tenstnatsatpair)
        {
            if (tenstnatsatpair != "tenstnatsatpair")
                return BadRequest("Parameter must be 'tenstnatsatpair'.");

            var foundRows = await _context.Table1
                .Where(c => TensNumbers.Contains(c.Am) && TNatsatNumbers.Contains(c.Pm))
                .OrderBy(c => c.Id)
                .ToListAsync();
        ...
        [HttpGet("weeksettenstnatsatpair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchWeekSets(string tenstnatsatpair, string day)
        {
            if (tenstnatsatpair != "tenstnatsatpair")
                return BadRequest("Parameter must be 'tenstnatsatpair'.");
            ...
            var foundRows = await _context.Table1
                .Where(c => c.Days == day && TensNumbers.Contains(c.Am) && TNatsatNumbers.Contains(c.Pm))
                .OrderBy(c => c.Id)
                .ToListAsync();
