...
namespace TwoDPro3.Controllers
{
    ...
    public class TNatsatTensPairController : Controller
    {
        private readonly CalendarContext _context;

        public TNatsatTensPairController(CalendarContext context)
        {
            _context = context;
        }
        ...
        [HttpGet("alldaytnatsattenspair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchAllDays(string tnatsattenspair)
        {
            if (tnatsattenspair != "tnatsattenspair")
                return BadRequest("Parameter must be 'tnatsattenspair'.");

            var foundRows = await _context.Table1
                .Where(c => TNatsatNumbers.Contains(c.Am) && TensNumbers.Contains(c.Pm))
                .OrderBy(c => c.Id)
                .ToListAsync();
        ...
        [HttpGet("weeksettnatsattenspair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchWeekSets(string tnatsattenspair, string day)
        {
            if (tnatsattenspair != "tnatsattenspair")
                return BadRequest("Parameter must be 'tnatsattenspair'.");
            ...
            var foundRows = await _context.Table1
                .Where(c => c.Days == day && TNatsatNumbers.Contains(c.Am) && TensNumbers.Contains(c.Pm))
                .OrderBy(c => c.Id)
                .ToListAsync();
