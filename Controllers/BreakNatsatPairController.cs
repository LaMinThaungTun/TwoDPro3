...
namespace TwoDPro3.Controllers
{
    ...
    public class BreakNatsatPairController : Controller
    {
        private readonly CalendarContext _context;

        public BreakNatsatPairController(CalendarContext context)
        {
            _context = context;
        }
        ...
        // ================= ALL DAYS =================
        [HttpGet("alldaybreaknatsatpair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchAllDays(string breaknatsatpair)
        {
            if (breaknatsatpair != "breaknatsatpair")
                return BadRequest("Parameter must be 'breaknatsatpair'.");
            ...
            var foundRows = await _context.Table1
                .Where(c =>
                    ...
                    (
                        (c.AmBreak == "0" && c.PmBreak == "7") || (c.AmBreak == "7" && c.PmBreak == "0") ||
                        (c.AmBreak == "1" && c.PmBreak == "8") || (c.AmBreak == "8" && c.PmBreak == "1") ||
                        (c.AmBreak == "2" && c.PmBreak == "4") || (c.AmBreak == "4" && c.PmBreak == "2") ||
                        (c.AmBreak == "3" && c.PmBreak == "5") || (c.AmBreak == "5" && c.PmBreak == "3") ||
                        (c.AmBreak == "6" && c.PmBreak == "9") || (c.AmBreak == "9" && c.PmBreak == "6")
                    )
                )
                .OrderBy(c => c.Id)
                .ToListAsync();
            ...
            if (!foundRows.Any())
                return NotFound("No matching break natsat pairs found.");
        ...
        // ================= WEEK SETS =================
        [HttpGet("weeksetsbreaknatsatpair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchWeekSets(string breaknatsatpair, string day)
        {
            if (breaknatsatpair != "breaknatsatpair")
                return BadRequest("Parameter must be 'breaknatsatpair'.");
