...
namespace TwoDPro3.Controllers
{
    ...
    public class BreakTPowerPairController : Controller
    {
        private readonly CalendarContext _context;

        public BreakTPowerPairController(CalendarContext context)
        {
            _context = context;
        }
        ...
        // ================= ALL DAYS =================
        [HttpGet("alldaybreaktpowerpair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchAllDays(string breaktpowerpair)
        {
            if (breaktpowerpair != "breaktpowerpair")
                return BadRequest("Parameter must be 'breaktpowerpair'.");
            ...
            var foundRows = await _context.Table1
                .Where(c =>
                    ...
                    (
                        (c.AmBreak == "0" && c.PmBreak == "9") || (c.AmBreak == "9" && c.PmBreak == "0") ||
                        (c.AmBreak == "1" && c.PmBreak == "3") || (c.AmBreak == "3" && c.PmBreak == "1") ||
                        (c.AmBreak == "2" && c.PmBreak == "6") || (c.AmBreak == "6" && c.PmBreak == "2") ||
                        (c.AmBreak == "4" && c.PmBreak == "7") || (c.AmBreak == "7" && c.PmBreak == "4") ||
                        (c.AmBreak == "5" && c.PmBreak == "8") || (c.AmBreak == "8" && c.PmBreak == "5")
                    )
                )
                .OrderBy(c => c.Id)
                .ToListAsync();
            ...
            if (!foundRows.Any())
                return NotFound("No matching break TPower pairs found.");
        ...
        // ================= WEEK SETS =================
        [HttpGet("weeksetsbreaktpowerpair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchWeekSets(string breaktpowerpair, string day)
        {
            if (breaktpowerpair != "breaktpowerpair")
                return BadRequest("Parameter must be 'breaktpowerpair'.");
