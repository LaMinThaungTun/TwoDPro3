...
namespace TwoDPro3.Controllers
{
    ...
    public class TensTPowerPairController : Controller
    {
        private readonly CalendarContext _context;

        public TensTPowerPairController(CalendarContext context)
        {
            _context = context;
        }
        ...
        [HttpGet("alldaytenstpowerpair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchAllDays(string tenstpowerpair)
        {
            if (tenstpowerpair != "tenstpowerpair")
                return BadRequest("Parameter must be 'tenstpowerpair'.");

            var foundRows = await _context.Table1
                .Where(c => TensNumbers.Contains(c.Am) && TPowerNumbers.Contains(c.Pm))
                .OrderBy(c => c.Id)
                .ToListAsync();
        ...
        [HttpGet("weeksettenstpowerpair")]
        public async Task<ActionResult<List<List<Calendar>>>> SearchWeekSets(string tenstpowerpair, string day)
        {
            if (tenstpowerpair != "tenstpowerpair")
                return BadRequest("Parameter must be 'tenstpowerpair'.");
            ...
            var foundRows = await _context.Table1
                .Where(c => c.Days == day && TensNumbers.Contains(c.Am) && TPowerNumbers.Contains(c.Pm))
                .OrderBy(c => c.Id)
                .ToListAsync();
